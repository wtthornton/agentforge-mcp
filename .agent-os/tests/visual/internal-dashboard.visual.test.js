...
describe('Internal Unified Dashboard Visual', () => {
  ...
  beforeAll(async () => {
    ...
    ensureDir(SNAP_DIR);
    ensureDir(DIFF_DIR);
    proc = startNodeProcess(path.join(INTERNAL, 'start-dashboard.js'), {}, [String(PORT)]);
  ...
  it('renders internal dashboard stable layout', async () => {
    await page.goto(BASE, { waitUntil: 'networkidle0' });
    const shotPath = path.join(DIFF_DIR, 'internal-dashboard-home.png');
    await page.screenshot({ path: shotPath, fullPage: true });

    const baselinePath = path.join(SNAP_DIR, 'internal-dashboard-home.png');
    ...
    const diffPath = path.join(DIFF_DIR, 'internal-dashboard-home-diff.png');
